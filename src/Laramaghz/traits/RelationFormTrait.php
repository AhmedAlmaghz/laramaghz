...
trait RelationFormTrait
{
    ...
    protected function appendToForm($oneToOneRelation , $type){
        ...
        foreach ($oneToOneRelation as $relation){
            ...
            $module = Str::singular(mb_strtolower($relation->module_to->name));
    ...
    protected function getLabelAndArray($relation){
        ...
        $array = '\\App\\Modules\\'.$relation->module_to->name.'\\Models\\'.Str::singular($relation->module_to->name).'::active()->pluck('.$pluckName.', "'.$relation->module_to->getkeyName().'")->toArray()';
    ...
    protected function appendInput($relation , $name ){
        ...
        if($relation->input_type == 'select'){
            return '@include("laramaghz::fileds.php.select" , [  "label"  => trans("'.$info['label'].'") , "array" => '.$info['array'].'  , "value" => $row->' . $name . ' ?? null'.'  ,  "name" =>"' . $name . '" ])'."\n";
        }elseif ($relation->input_type == 'checkbox'){
            return '@include("laramaghz::fileds.php.checkbox" , [  "label"  => trans("'.$info['label'].'") , "array" => '.$info['array'].'   , "selectedArray" => $row->' . $name . ' ?? null'.'  ,  "name" =>"' . $name . '" ])'."\n";
        }
    }
    ...
    protected function appendInputManyToMany($relation , $name  , $module){
        ...
        if($relation->input_type == 'select'){
            return '@include("laramaghz::fileds.php.select" , [  "label"  => trans("'.$info['label'].'") , "array" => '.$info['array'].'  , "value" => $row->' . $module . $pluck. ' ?? null'.'  ,  "name" =>"' . $name . '[]" , "multiSelect" => true ])'."\n";
        }elseif ($relation->input_type == 'checkbox'){
            return '@include("laramaghz::fileds.php.checkbox" , [  "label"  => trans("'.$info['label'].'") , "array" => '.$info['array'].'   , "selectedArray" => $row->' . $module .$pluck . ' ?? null'.'  ,  "name" =>"' . $name . '[]" ])'."\n";
        }
    }
